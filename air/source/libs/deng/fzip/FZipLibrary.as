...
package deng.fzip {
	...
	/**
	 * FZipLibrary processes files (based on file extensions) from an
	 * FZip instance and converts them into usable formats. Files can
	 * be converted to either a BitmapData or DisplayObject classes.
	 * Data embedded in SWF files (like classes) can also be retrieved.
	 * 
	 * Flash's built-in Loader class is used to convert formats, so the
	 * only formats currently supported are ones that Loader supports.
	 * As of this writing they are SWF, JPEG, GIF, and PNG.
	 * 
	 * The following example loads an external zip file, outputs the
	 * width and height of an image and then loads a sound from a SWF file.
	 * 
	 * <code>
	 * package {
	 * 	import flash.events.*;
	 ...
	 * 			lib = new FZipLibrary();
	 * 			files.autoFormatBitmapData(".gif");
	 * 			files.autoFormatBitmapData(".jpg");
	 * 			files.autoFormatBitmapData(".png");
	 * 			files.autoFormatDisplayObject(".swf");
	 * 			lib.addEventListener(Event.COMPLETE,onLoad);
	 * 			lib.addZip(new FZip(true,url));
	 * 		}
	 ...
	 * 			var snd:Sound = new importedSound();
	 * 		}
	 * 	}
	 * }
	 * </code>
	...
	public class FZipLibrary extends EventDispatcher {
		 ...
		 * @throws Error If the file under that name is not available as
		 * a BitmapData class, an error is thrown
		 */
		public function getBitmapData(filename:String):BitmapData {
			if (!bitmapDataList.hasOwnProperty(filename)) {
		 ...
		 * @throws ReferenceError If the file under that name is not
		 * available as a DisplayObject class, an error is thrown.
		 */
		 ...
		 * @param filename The filename of the DisplayObject instance.
		 * @throws ReferenceError If the file under that name is not
		 * available as a DisplayObject class or the definition does not
